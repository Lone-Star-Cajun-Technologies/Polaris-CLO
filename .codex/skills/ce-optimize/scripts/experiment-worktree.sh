...
create_worktree() {
  ...
  for shared_file in "$@"; do
    if [[ -z "$shared_file" || "$shared_file" = /* || "$shared_file" == *".."* ]]; then
      echo -e "${RED}Error: Invalid shared path: $shared_file${NC}" >&2
      return 1
    fi

    local src="$GIT_ROOT/$shared_file"
    local dst="$worktree_path/$shared_file"
    local dst_dir
    dst_dir=$(dirname "$dst")
    mkdir -p "$dst_dir"

    local canonical_dst_dir canonical_worktree canonical_gitroot
    canonical_dst_dir=$(cd "$dst_dir" && pwd -P)
    canonical_worktree=$(cd "$worktree_path" && pwd -P)
    canonical_gitroot=$(cd "$GIT_ROOT" && pwd -P)

    case "$canonical_dst_dir" in
      "$canonical_worktree"|"$canonical_worktree"/*) ;;
      *)
        echo -e "${RED}Error: Shared path escapes worktree: $shared_file${NC}" >&2
        return 1
        ;;
    esac
    local canonical_dst_path="${canonical_dst_dir}/$(basename "$dst")"
    case "$canonical_dst_path" in
      "$canonical_worktree"/*) ;;
      *)
        echo -e "${RED}Error: Shared destination escapes worktree: $shared_file${NC}" >&2
        return 1
        ;;
    esac
    ...
    if [[ -f "$src" ]]; then
      ...
      if [[ "$canonical_src" != "$canonical_gitroot/"* ]]; then
        echo -e "${RED}Error: Shared source escapes repository root: $shared_file${NC}" >&2
        return 1
      fi
      cp "$src" "$dst"
    elif [[ -d "$src" ]]; then
      ...
      if [[ -z "$canonical_dst_path" || "$canonical_dst_path" = "/" || "$canonical_dst_path" = "." ]]; then
        echo -e "${RED}Error: Refusing unsafe destination delete path: $shared_file${NC}" >&2
        return 1
      fi
      case "$canonical_dst_path" in
        "$canonical_worktree"/*) ;;
        *)
          echo -e "${RED}Error: Destination delete path is out of bounds: $shared_file${NC}" >&2
          return 1
          ;;
      esac
