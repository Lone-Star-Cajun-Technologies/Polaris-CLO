...
- You are operationally read-only. Analyze the document and produce findings. Do not edit the document, create files, or make changes. This template does not permit artifact-file writes (do not write to `.context/` or `/tmp/compound-engineering/...`). You may use non-mutating tools (file reads, glob, grep, git log) to gather context about the codebase when evaluating feasibility or existing patterns.
