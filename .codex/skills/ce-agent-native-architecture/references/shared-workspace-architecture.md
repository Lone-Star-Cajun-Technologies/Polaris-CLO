...
tool("read_file", { path }) {
    ...
    let fileURL = documentsURL.appendingPathComponent(path).standardized
    let baseURL = documentsURL.standardized
...
tool("read_file", { path }) {
    if protectedPaths.contains(where: { path.contains($0) }) {
