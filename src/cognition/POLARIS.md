...
The cognition subsystem provides delta-only logic for maintaining route-local `POLARIS.md` and `SUMMARY.md` surfaces. It determines whether a cognition update is warranted after a child completes, detects missing surfaces, validates size and doctrine constraints, and generates seed drafts. It does not write cognition files itself — callers (`worker.ts`, map update, smartdocs ingest) use these signals to decide when and what to write.
...
- Writing `POLARIS.md` or `SUMMARY.md` files — callers own that responsibility.
...
- Delta functions return signals; they do not write files. Keep it that way.
