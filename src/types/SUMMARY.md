...
## Purpose
Shared TypeScript type definitions for Polaris runtime contracts. Cross-route interfaces for worker result packets, run-health symptom reporting, Medic run-health consult, tracker adapters, and runtime state.

## Core Concepts
- `ResultPacket` is a discriminated union (`SuccessResultPacket | FailedResultPacket`) — the authoritative contract between workers and the Foreman, and the source of run-health symptoms consumed by Medic.
...
- `MedicRunHealthResult` is the Medic's return contract after a run-health consult.
