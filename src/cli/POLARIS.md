...
- `polaris adopt consolidate --dry-run` must perform zero writes to the working tree: `migrateSmartDocs()` and `reconcileAgentFiles()` gate every `git mv`/`renameSync`/`mkdirSync`/`writeFileSync`/provenance write behind `plan.dry_run`, logging `[dry-run] would ...` lines instead.
