...
function buildPolarisDraft(folder: string): string {
  ...
  return [
    ...
    "- Do NOT use this file as a run diary; per-run notes belong in the run artifact or `SUMMARY.md`.",
...
function buildSummaryDraft(folder: string): string {
  ...
  return [
    ...
    "## Responsibilities",
    "",
    "<!-- Concise context, references, ownership, and recent state for this folder. -->",
    "",
    "## Constraints and exclusions",
    "",
    "- Do NOT include operational procedures or step-by-step agent instructions; those belong in `POLARIS.md`.",
    "- Do NOT duplicate doctrine content; link to `POLARIS.md` or `smartdocs/doctrine/active/` instead.",
    "- Keep this a lightweight memory surface, not a run log or task tracker.",
    "",
  ].join("\n");
}
